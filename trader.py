...
async def trade_signal_webhook(request: Request) -> JSONResponse:
    ...
    try:
        ...
        for i in range(3): # Try a few times to get initial order details
            try:
                initial_order_details = await pocket_option_client.check_order_result(order.order_id)
                if initial_order_details and initial_order_details.open_price:
                    trade_sequence_state["last_trade_open_price"] = initial_order_details.open_price
                    trade_sequence_state["last_trade_open_time"] = initial_order_details.open_time
                    logger.info(f"Initial trade open price obtained: {initial_order_details.open_price} at {initial_order_details.open_time}")
                    break
            except Exception as e:
...
async def handle_trade_outcome_and_martingale(trade_id: int, duration: int, asset: str, direction: OrderDirection, amount: float, trade_open_price: Optional[float]) -> None:
        ...
        if trade_sequence_state["current_level"] < MAX_MARTINGALE_LEVELS:
            ...
            try:
                ...
                for i in range(3): # Try a few times to get new order details
                    try:
                        martingale_order_details = await pocket_option_client.check_order_result(next_order.order_id)
                        if martingale_order_details and martingale_order_details.open_price:
                            trade_sequence_state["last_trade_open_price"] = martingale_order_details.open_price
                            trade_sequence_state["last_trade_open_time"] = martingale_order_details.open_time
                            logger.info(f"Martingale trade open price obtained: {martingale_order_details.open_price} at {martingale_order_details.open_time}")
                            break
                    except Exception as e:
    ...
    if pocket_option_client and pocket_option_client.is_connected:
        try:
            final_trade_details = await pocket_option_client.check_order_result(trade_id)
